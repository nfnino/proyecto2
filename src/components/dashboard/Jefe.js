    ...
    render() {
        ...
        if (res == null || tasksLoading) {
            ...
            for(let i=0; i<res.length; i++) {
                if(res[i].supervisor === auth.user.name){
            ...
            dashboardContent = <Table className={classes.table} aria-label="simple table">
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            <TableCell align="right"><Typography color="primary"> Ejecutor</Typography></TableCell>
                                    ...
                                    <TableBody>{array.map((item) => ( 
                                        <TableRow key={item._id}>
                                            ...
                                            <TableCell align="right">{`${item.ejecutor_interno}`}</TableCell>
                                            <TableCell align="right">{item.estado!=="Creada" 
                                                                    ? <Typography color="error">{item.estado}</Typography> 
                                                                    : <Typography>{item.estado}</Typography>}
                                            </TableCell>
                                            <TableCell align="right"><Button href={`/tasks/${item._id}`} variant="contained" color="primary"> Detalles </Button></TableCell>                          
                                        </TableRow>
                                    ))}
                                    </TableBody>
                                </Table>
